...
namespace LeaRun.Utilities
{
    ...
    public class DateTimeHelper
    {
        ...
        {
            int resultcount = 0;
            for (int i = 1; i < GetDaysOfMonth(dt) + 1; i++)
            {
                var date = DateTime.Parse(dt.Year.ToString() + "_" + dt.Month.ToString() + i.ToString());
        ...
        //获取当月工作日有多多少天
        public int GetWorkDays(DateTime dt)
        {
            int resultcount = 0;
            for (int i = 1; i < GetDaysOfMonth(dt) + 1; i++)
            {
       ...
       // 获取当月周末工作日
        public static  List<int> GetMonthArr(DateTime dt)
        {
            var list = new List<int>();
            for (int i = 1; i < GetDaysOfMonth(dt) + 1; i++)
            {
                var date = DateTime.Parse(dt.Year.ToString() + "-" + dt.Month.ToString()+"-" + i.ToString());
                if (date.DayOfWeek == DayOfWeek.Sunday || date.DayOfWeek == DayOfWeek.Saturday)
                {
                    list.Add(1);
                }
