...
namespace MallWCF.DBHelper
{
    public class DatabaseCommon
    {
        ...
        public static DbParameter[] GetParameter<T>(T entity)
        {
            ...
            {
                count++;
                if (pi.GetValue(entity, null) != null&&count>1)
                {
        ...
        //修改使用
        public static DbParameter[] GetParameterNew<T>(T entity)
        {
            IList<DbParameter> parameter = new List<DbParameter>();
            DbType dbtype = new DbType();
            Type type = entity.GetType();
        ...
        public static StringBuilder InsertSql<T>(T entity)
        {
            ...
            {
                count++;
                if (prop.GetValue(entity, null) != null && count>1)
                {
            ...
            sb.Append(sb_prame.ToString().Substring(1, sb_prame.ToString().Length - 1) + ") Values (");
            sb.Append(sp.ToString().Substring(1, sp.ToString().Length - 1) + ");select @@IDENTITY");
            return sb;
        }
        #endregion
